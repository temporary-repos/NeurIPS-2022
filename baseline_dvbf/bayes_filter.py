...
class BayesFilter():
    def __init__(self, args):
        ...
        self.x = tf.placeholder(dtype=np.float32, name="state_values")
        self.feat_x = tf.placeholder(shape=[args.batch_size * args.seq_length, args.state_dim], dtype=np.float32, name='feat_x')
        self.u = tf.placeholder(dtype=np.float32, name="action_values")
    ...
    def _create_feature_extractor(self, args):
        features = self._get_extractor_output(args, self.feat_x)
        self.features = tf.reshape(features, [args.batch_size, args.seq_length, args.code_dim])
    ...
    def _create_initial_generator(self, args):
        ...
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(fwd_cell, bwd_cell, self.features[:, :args.z_start_len],
                                                     dtype=tf.float32)
    ...
    def test_prop(self, args, z_t):
        print("In test set.")
        z_pred = [z_t]
        w_dists = [tf.expand_dims(self.w1_dist, axis=1)]

        # Generate garbage controls as they aren't used
        u_ts = tf.zeros((args.batch_size, args.trial_len - 1, 1), dtype=tf.float32)

        # Loop over length of trial
        for t in range(1, args.trial_len):
            ...
            w_dists.append(tf.expand_dims(tf.ones_like(self.w1_dist), axis=1))
        ...
        z_pred = tf.reshape(tf.stack(z_pred, axis=1), [args.batch_size * args.trial_len, args.code_dim])
        w_dists = tf.reshape(tf.stack(w_dists, axis=1), [args.batch_size * args.trial_len, 2 * args.noise_dim])
        return z_pred, w_dists

    def train_prop(self, args, z_t):
        print("In train.")
        ...
        for t in range(1, args.seq_length):
            ...
            w_dists.append(tf.expand_dims(w_dist, axis=1))
        ...
        # Stack predictions into single tensor for reconstruction
        z_pred = tf.reshape(tf.stack(z_pred, axis=1), [args.batch_size * args.seq_length, args.code_dim])
        w_dists = tf.reshape(tf.stack(w_dists, axis=1), [args.batch_size * args.seq_length, 2 * args.noise_dim])
        return z_pred, w_dists
    ...
    def _propagate_solution(self, args):
        ...
        # Set whether it is the training or testing loops
        self.z_pred, self.w_dists = tf.cond(self.is_train, lambda: self.train_prop(args, z_t), lambda: self.test_prop(args, z_t))
        # self.test_prop(args, z_t)
    ...
    def _create_decoder_params(self, args):
        ...
        for i in range(len(args.extractor_size) - 1, -1, -1):
            if i == len(args.extractor_size) - 1:
                ...
                prev_size = args.extractor_size[i + 1]
            self.decoder_w.append(
                tf.get_variable("decoder_w" + str(len(args.extractor_size) - i), [prev_size, args.extractor_size[i]],
                                regularizer=tf.contrib.layers.l2_regularizer(args.reg_weight)))
            self.decoder_b.append(
                tf.get_variable("decoder_b" + str(len(args.extractor_size) - i), [args.extractor_size[i]]))
    ...
    def _get_decoder_output(self, args, encodings):
        ...
        output = tf.nn.sigmoid(tf.nn.xw_plus_b(decoder_input, self.decoder_w[-1], self.decoder_b[-1]))
        return output
    ...
    def _create_optimizer(self, args):
        ...
        self.cost = self.loss_reconstruction + tf.reduce_sum(
            tf.losses.get_regularization_losses()) + self.kl_weight * self.kl_loss
