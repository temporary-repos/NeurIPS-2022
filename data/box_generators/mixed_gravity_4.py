...
if __name__ == '__main__':
    ...
    for idx, g in enumerate(gs):
        ...
        cannon = BallBox(dt=0.2, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=8, wall=None,
                            gravity=(g_x, g_y))
        i, s = cannon.run(delay=None, iterations=60, sequences=sequences, radius=3*scale, angle_limits=(0, 360),
                            velocity_limits=(10.0*scale, 10.0*scale), save='npz')
        images.append(i)
        states.append(s)
        labels.append(np.full(sequences, idx))

        images = np.concatenate(images, axis=0)
        states = np.concatenate(states, axis=0)
        labels = np.concatenate(labels, axis=0)

        np.savez(os.path.abspath("../bouncing_ball/mixed_gravity_4_set{}.npz".format(idx)), images=images, state=states, label=labels)
